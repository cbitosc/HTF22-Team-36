...
def add(id):
    if request.method == "POST":
        listItems = ListItems(title = request.form['title'],content =  request.form['content'],  deadline = datetime.strptime(request.form['deadline'],"%Y-%m-%dT%H:%M") ,reference=id, username = read_File() )
...
def sign_in():
    ...
    if request.method == "POST":
        ...
        if (users):
            write_File(request.form['username'])
            return redirect('/index')
        else:
            status = False
...
def index():
    username = read_File()
    if request.method=="POST":
         kanBanList = KanBanList(listName = request.form['name'], username = username, sno = get_Count()) 
         try:
            db.session.add(kanBanList)
            db.session.commit()
         except:
    ...
    allLists = KanBanList.query.all()
    listItems = ListItems.query.filter_by(username=username).all()
...
def displayStats():
    ...
    listItems = ListItems.query.filter_by(username = read_File())
